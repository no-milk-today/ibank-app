...
- **Config Server**  
  Externalized/Distributed Config. It uses Spring Cloud Config to serve configuration properties from local files.
  **Config Clients:** `customer`, `exchange-generator`, `notification` (to demonstrate externalized configuration pattern).
...
- **Keycloak** (OIDC, port 8090, realm: `bank-realm`) provides authentication and JWT tokens.
...
- **Config Server**: `8888` - Externalized configuration management

## Docker Deployment
The project uses **JIB Maven Plugin** for containerization and **Docker Compose** for orchestration. 
All microservices are configured to run in Docker containers with proper service discovery and networking.


## Running the Project
#### todo: ```mvn clean package -P build-docker-image```
### ```docker compose pull```
### ```docker compose up -d```

#### to avoid pushing the image: ```mvn clean package```
1. ```docker-compose up -d```. Then import realm configuration from `keycloak/config/realm-export.json` into Keycloak.
   ...
   - Navigate to `Clients` → `front-ui-admin` → `Service accounts roles`. 
   - **Add the following roles:** `realm-management → manage-users`, `realm-management → view-users`

2. connect to pgadmin and create a databases:
   ```
...
4. Build the project:
   ...
   mvn clean install
    ```
5. Run the services (start the Eureka Server first, then the config-server and then the others):
   ```bash
   mvn spring-boot:run
    ```
6. Access the application (api-gateway) at `http://localhost:8080`.
7. Register new customer `http://localhost:8080/signup`.
8. Do logout (via `http://localhost:8080/logout` and create one more customer (to test transferring money between them).
9. Access the Eureka dashboard at `http://localhost:8761` to see all registered services.
...
minikube service keycloak-nodeport